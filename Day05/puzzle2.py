...
class Map:
    def __init__(self, x: int, y: int):
        self.map = [[0 for _ in range(y + 1)] for _ in range(x + 1)]
        self.x_max = x
        self.y_max = y

    def update(self, line: Line) -> None:
        if line.y1 == line.y2:
            for x in range(line.x1 - 1, line.x2 - 2 * int(line.x1 > line.x2),
                           1 if line.x1 < line.x2 else -1):
                self.map[x][line.y1 - 1] += 1
        elif line.x1 == line.x2:
            for y in range(line.y1 - 1, line.y2 - 2 * int(line.y1 > line.y2),
                           1 if line.y1 < line.y2 else -1):
                self.map[line.x1 - 1][y] += 1
        else:
            for x, y in zip(range(line.x1 - 1,
                                  line.x2 - 2 * int(line.x1 > line.x2),
                                  1 if line.x1 < line.x2 else -1),
                            range(line.y1 - 1,
                                  line.y2 - 2 * int(line.y1 > line.y2),
                                  1 if line.y1 < line.y2 else -1)):
                self.map[x][y] += 1

    def count(self, flt: typing.Callable[[int], bool]) -> int:
        return sum([sum([1 for p in line if flt(p)]) for line in self.map])
